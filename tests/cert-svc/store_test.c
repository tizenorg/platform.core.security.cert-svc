...
int tcase_1_success()
{
	...
	if(ret == CERT_SVC_ERR_NO_ERROR) {
		if((access("/usr/share/cert-svc/certs/code-signing/java/thirdparty/Broot.der", F_OK)) != 0)	// fail
			return -1;
		else
			return 0;
	}
...
int tcase_2_success()
{
	...
	if(ret == CERT_SVC_ERR_NO_ERROR) {
		if((access("/usr/share/cert-svc/certs/ssl/Broot.der", F_OK)) != 0)	// fail
			return -1;
		else
			return 0;
	}
...
int finalize_test()
{
	...
	if((unlink("/usr/share/cert-svc/certs/code-signing/java/thirdparty/Broot.der")) != 0)	// fail
		return -1;
	if((unlink("/usr/share/cert-svc/certs/ssl/Broot.der")) != 0)	// fail
		return -1;
