	...
	int *validity)
{
	...
	tempNode = sorted; // first item is the certificate that user want to verify

	// insert all certificate(s) into chain
	if (sorted != NULL) { // has 2 or more certificates
		interCert = (X509**)malloc(sizeof(X509*) * certNum);
		if (interCert == NULL) {
			SLOGE("Failed to allocate memory for interim certificate.");
			ret = CERT_SVC_ERR_MEMORY_ALLOCATION;
			goto err;
		}
		...
		for (tempNode = sorted, certIndex = 0;
			...
			if (!_d2i_X509(tempNode->certificate, &interCert[certIndex])) {
