	...
	SignatureData &data)
{
	...
	if (!collection.sort() || collection.empty() || collection.completeCertificateChain()) {
		...
		return SignatureValidator::SIGNATURE_INVALID_CERT_CHAIN;
	}
	...
	LogDebug("root certificate from " << storeIdSet.getTypeString() << " domain");
	if (data.isAuthorSignature()) {
		if (storeIdSet.getType() != CertStoreId::TIZEN_DEVELOPER) {
		...
		if (storeIdSet.getType() == CertStoreId::TIZEN_DEVELOPER) {
			...
			return SignatureValidator::SIGNATURE_IN_DISTRIBUTOR_CASE_AUTHOR_CERT;
		}

		if (data.getSignatureNumber() == 1 && !storeIdSet.isVisibilityLevel()) {
	...
	if (X509_cmp_time(notBeforeTime, &nowTime) > 0  || X509_cmp_time(notAfterTime, &nowTime) < 0) {
		if (storeIdSet.getType() == CertStoreId::TIZEN_TEST || storeIdSet.getType() == CertStoreId::TIZEN_VERIFY) {
	...
	SignatureData &outData)
{
	...
	if (!outData.isAuthorSignature()) {
		if (XmlSec::NO_ERROR != XmlSecSingleton::Instance().validate(&context)) {
			...
			return SIGNATURE_INVALID_HASH_SIGNATURE;
		}
		...
		if (checkReferences) {
			...
			if (ReferenceValidator::NO_ERROR != fileValidator.checkReferences(outData)) {
				...
				return SIGNATURE_INVALID_NO_HASH_FILE;
			}
		}
	}

	return disregard ? SIGNATURE_INVALID_DISTRIBUTOR_CERT : SIGNATURE_VERIFIED;
}
