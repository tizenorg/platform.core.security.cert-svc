...
typedef std::unique_ptr<X509_STORE, void(*)(X509_STORE*)> X509_STORE_PTR;
typedef std::unique_ptr<SSL_CTX, void(*)(SSL_CTX*)> SSL_CTX_PTR;
typedef std::unique_ptr<BIO, void(*)(BIO*)> BIO_PTR;
typedef std::unique_ptr<char, void(*)(void*)> RAIIstr;
typedef std::unique_ptr<OCSP_REQUEST, void(*)(OCSP_REQUEST*)> OCSP_REQUEST_PTR;
typedef std::unique_ptr<OCSP_RESPONSE, void(*)(OCSP_RESPONSE*)> OCSP_RESPONSE_PTR;
typedef std::unique_ptr<OCSP_BASICRESP, void(*)(OCSP_BASICRESP*)> OCSP_BASICRESP_PTR;
	...
	X509_STACK_PTR &trustedCerts)
{
	...
	int response = OCSP_basic_verify(basicResp.get(), NULL, trustedStore.get(), 0);
	if (response <= 0)
	...
	if (ocspStatus == V_OCSP_CERTSTATUS_REVOKED)
		return Ocsp::Result::REVOKED;
	else if (ocspStatus != V_OCSP_CERTSTATUS_GOOD)
		VcoreThrowMsg(Ocsp::Exception::InvalidResponse, "Unknown ocsp status.");
	else
		return Ocsp::Result::GOOD;
}
...
Ocsp::Result Ocsp::check(const SignatureData &data)
{
	...
	for (; it != certChain.cend(); it++) {
		if (it->get()->getDER().empty())
			VcoreThrowMsg(Exception::InvalidParam, "Broken certificate chain.");

		sk_X509_push(trustedCerts.get(), it->get()->getX509());
	}
	...
	itIssuer++;
	for (size_t idx = 0; idx < certChain.size() - 1; idx++, itCert++, itIssuer++)
